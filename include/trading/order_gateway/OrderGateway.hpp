...
namespace trading::order_gateway
{
    ...
    {
        public:
            OrderGateway(ClientId clientId, ClientRequestLFQueue* clientRequests, ClientResponseLFQueue* clientResponses, const std::string& ip, const std::string& iface, int port);
            ~OrderGateway();
